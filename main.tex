...
\title{Symmtries in Wick Rotated QFT}
...
\[
  ...
  \longrightarrow \mathcal{H}_{1,n-1},
\]
i.e.\ the usual action of the (spin cover of the) Poincaré group on the full symmetry group of the Minkowski theory.  In particular, the statement that $-1\in\Spin_n$ maps to $k_0\in K$ becomes, after anti-Wick rotation, the familiar
fact that the central element of the Lorentz spin group can act as an internal symmetry (e.g.\ $(-1)^F$) on the theory.\\
...
\[H=\varinjlim_{n}H_n,\qquad
    \rho:H\longrightarrow O=\varinjlim_n O_n.
\]
...
\[
    ...
    {\rm tVect}_{\mathbb C}.
\]
The bordism category records the geometric processes of the theory, while the functor $F$ assigns to them the corresponding physical data: state spaces, evolution maps, partition functions, and correlation functions. The symmetry group tells us how this functor behaves under changes of the background geometry. In particular, a symmetry type is a pair
\[
    (H,\rho),
    \qquad
    \rho:H\longrightarrow O,
\]
where $O$ is the stable Euclidean spacetime symmetry group and $H$ contains both the spacetime symmetries and the internal symmetries. The kernel
\[
    K=\ker(\rho)
\]
consists of symmetries which act trivially on spacetime, while the homomorphism $\rho$ records how the remaining symmetries act on the background geometry. Thus the symmetry type is additional structure on the field theory: it specifies which transformations of the geometric background are
symmetries of the theory and how these transformations are lifted to the physical system. The stabilization
\[
    H_n\hookrightarrow H_{n+1}\hookrightarrow\cdots
\]
allows us to regard these symmetry data as dimension-independent:
\[
    H=\varinjlim_n H_n,
    \qquad
    \rho:H\longrightarrow
    O=\varinjlim_n O_n.
\]
Consequently, instead of studying a different symmetry group in every dimension, we can study a single stable symmetry type $(H,\rho)$ and
recover its finite-dimensional manifestations by pullback. The overall picture of the lecture is therefore
\[
\mathbb M^n
\rightsquigarrow
\mathcal D
...
X^n
\]
The role of the symmetry type is to constrain and organize the functorial data of the field theory. The bordism category tells us what geometric processes are possible, the functor $F$ tells us what physical data those processes produce, and the symmetry type $(H,\rho)$ tells us which spacetime and internal transformations preserve the theory. In this sense, the passage from Minkowski space through Wick rotation to a stable Euclidean symmetry type is not an additional piece of
structure unrelated to the bordism formulation.  Rather, it explains the symmetry data that the functorial description of a quantum field theory is required to respect.
...
\]
whose objects are $(n-1)$-dimensional manifolds equipped with differential
$H_n$-structure and whose morphisms are $n$-dimensional manifolds with
differential $H_n$-structure that provide bordisms between these
$(n-1)$-dimensional manifolds.\\

More concretely, an object of $\operatorname{Bord}_{\langle n-1,n \rangle}(H_n^\nabla)$ is a closed $(n-1)$-manifold $Y_0$ together with the geometric structure determined by
...
identified with
\[
\partial X \cong Y_0 \sqcup Y_1,
\]
together with a differential $H_n$-structure on $M$ which restricts to the
given structures on the boundary. Composition of morphisms is given by
gluing bordisms along their common boundary.\\

As discussed earlier, the functor $F$ encodes the basic physical data of the
Wick-rotated theory. The vector spaces $F(\Sigma)$ encode the spaces of
states associated to spatial manifolds, while the linear maps associated to bordisms encode the corresponding amplitudes and correlation functions.
Observables and their correlation functions can therefore be understood as
part of the functorial structure rather than as additional independent
axioms.\\

This completes the progression from ordinary relativistic quantum field
theory to the geometric formulation of the Axiom System. Schematically, the
successive enlargements of the class of allowed backgrounds are
\[
\mathbb{M}^n
\rightsquigarrow
\mathcal{D}^n
...
X^n,
\]
where $\mathbb{M}^n$ denotes Minkowski spacetime, $\mathcal{D}^n$ denotes the complexified setting, $\mathbb{E}^n$ denotes Euclidean space, and $X^n$ denotes a general curved $n$-manifold equipped with the appropriate differential $H_n$-structure.

One can therefore view the Axiom System as progressively removing the
dependence of the theory on a particular background. Instead of beginning
with a theory defined only on Minkowski space, we allow the theory to be
defined on increasingly general geometric objects. The symmetry type
$(H_n,\rho_n)$ specifies the local geometric structure that the theory is
allowed to see, while the bordism category organizes all such manifolds and
their gluings into a single categorical framework.\\

Ideally, one would like a reconstruction theorem which reverses the
progression done in this lecture  showing that sufficiently general geometric and categorical data determine the original physical theory. Such a theorem would provide a precise mathematical explanation for how the usual formulation of quantum field theory on Minkowski spacetime can be recovered from the more abstract Axiom System. At present, however, this should be regarded as a structural goal rather than a general theorem.
...
distinguished origin, but it has a simply transitive action of its translation
group $\mathbb{R}^n$. Its symmetry group is therefore the affine group
\[
...
$\mathbb{A}^n$ has a symmetry Lie group:
\[
\begin{tikzcd}
1 \arrow[r] &
\mathbb{R}^n \arrow[r] \arrow[d, equal] &
\mathcal{H}_n \arrow[r] \arrow[d] &
...
\]
defining the Cartan symmetry type of the geometric structure. In contrast to the quantum field theory setting in last section, $H_n$ need not be compact, and the target of $\rho_n$ is $\operatorname{GL}_n(\mathbb{R})$ rather than $O_n$.\\

The basic examples are summarized by the following table:
\[
\begin{array}{c|c|c}
...
\end{array}
\]
Internal symmetry groups can also be incorporated into this framework, though they are not done here or in [F].\\

The transition from affine to curved geometry can be understood in terms of
parallelism. Affine geometry has a global parallelism, encoded by the
simply transitive action of the translation group $\mathbb{R}^n$. On a general
manifold, there is no canonical way to identify tangent spaces at distinct
points. The curved analogue of global parallelism is therefore ``infinitesimal parallelism'', encoded by an affine connection. More
precisely, an affine connection on a smooth $n$-manifold $M$ is a connection
on the principal $\operatorname{GL}_n(\mathbb{R})$-bundle of frames
\[
\mathcal{B}(M)\longrightarrow M.
\]
It is natural to require the affine connection to be torsionfree. In the special case where the image of $\rho_n$ lies in $O_n$, the resulting geometry is
Riemannian and the unique torsionfree compatible connection is the Levi--Civita connection.

\begin{definition}
Let $M$ be a smooth $n$-manifold. The frame bundle
$\mathcal{B}(M)$ is the principal $\operatorname{GL}_n(\mathbb{R})$-bundle whose fiber over $p\in M$ is
\[
\mathcal{B}_p(M)
=
\left\{
(e_1,\ldots,e_n)
\;\middle|\;
(e_1,\ldots,e_n)\text{ is a basis of }T_pM
\right\}.
\]
\end{definition}

We can now replace the global affine symmetry by a local geometric structure
on a curved manifold. Let $(H_n,\rho_n)$ be a Cartan symmetry type, where
\[
\rho_n:H_n\longrightarrow\operatorname{GL}_n(\mathbb{R})
\]
is a homomorphism. The group $H_n$ specifies the allowed local symmetries,
while $\rho_n$ describes how these symmetries act on the tangent space.

\begin{definition}[Differential $H_n$-structure]
Let $(H_n,\rho_n)$ be a Cartan symmetry type. A \emph{differential
$H_n$-structure} on a smooth $n$-manifold $M$ is a triple
\[
(P,\Theta,\varphi),
\]
where $P\to M$ is a principal $H_n$-bundle equipped with a connection
$\Theta$, and
\[
\varphi:\mathcal{B}(M)\longrightarrow \rho_n(P)
\]
is an isomorphism of principal $\operatorname{GL}_n(\mathbb{R})$-bundles
such that the induced connection
\[
\varphi^*\rho_n(\Theta)
\]
is torsionfree.
...
group, while a differential $H_n$-structure replaces this global symmetry by
an infinitesimal symmetry acting on each tangent space. The study of
$H_n$-geometry is then the study of the invariants of smooth manifolds
equipped with such structures and their diffeomorphisms.
